...
class Report:
    ...
    def parse(self):
        ...
        wb = load_workbook(filename=self.file_name, read_only=True, data_only=True)
        ws = wb[self.worksheet_name]
        logger.debug(f'Slurp item information from worksheet "{ws.title}".')

        logger.debug(f"Max row of this sheet is {ws.max_row}.")
        for row_number, row in enumerate(
            ws.iter_rows(min_row=self.row_of_label + 1, max_row=ws.max_row + 1, values_only=True), start=self.row_of_label + 1
        ):
